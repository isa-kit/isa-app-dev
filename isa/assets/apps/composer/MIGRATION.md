...
## 2026-09-15 — Mirror accuracy stamp (boardhistory-0915)

**What changed.** Two new HIDDEN, additive columns on `boards`:
`mirrorApplet` (text) and `mirrorBuild` (text). Set by
`buildAppletMirrorTemplate` when a "Mirror an applet" board is created
(`mirrorApplet` = the source applet id, `mirrorBuild` = that applet's
`app.json` `seedVersion` at generation time) and refreshed by
`refreshAppletMirrorBoard`'s "Sync with applet" pass to the applet's
CURRENT `seedVersion`. Purely additive/nullable — an existing board (never
generated from an applet) simply has both fields null/absent, and no
existing row is rewritten. No data migration needed; `seedVersion` bump is
precautionary (columns.json changed) per the binding schema-change rule.
...
## 2026-09-05 — Wikipick "Seed map" design proposal (seedmap-prop-0905)

**What changed.** Adds a new native design-proposal board, `bd_wikipick_
seedmap` ("Wikipick — Seed map"), authored per `moku-native-proposals.md` in
response to the user's ask for a seed map visualizing saved articles and
degrees-away, with individually-saved articles vs. cached layer members
colored differently. Two `proposed` screens (`sc_wsm_map` "Seed map",
`sc_wsm_focus` "Node focus"), 10 widgets, 4 decision stickies (render-engine
choice, radial-no-jitter layout, derived-table-not-a-new-table data model,
nav placement). No `frameImages` rows — fully native from the start.

- `boards`: +1 row (`bd_wikipick_seedmap`, `mode: "design"`, `rev: 1`).
- `screens`: +2 rows, both `status: "proposed"`.
- `widgets`: +10 rows across the 2 screens (see
  `lib/src/composer_native_wikipick_seedmap_migration.dart`'s
  `composerWikipickSeedMapWidgets` map for the authoritative per-screen
  list — kept in lockstep with `content.json` by
  `test/composer_native_wikipick_seedmap_migration_test.dart`'s parity
  check). The graph/canvas area on both screens uses the `image` kind as a
  placeholder — no native `graph`/canvas-drawing kind exists yet; see
  sticky `st_wsm_engine_choice` for the recommended real render path.
- `stickies`: +4 rows (`st_wsm_engine_choice`, `st_wsm_layout_rule`,
  `st_wsm_data_model`, `st_wsm_placement`).
- `app.json`: `seedVersion` bumped to
  `2026-09-05.wikipick-seedmap-proposal`.
- No `columns.json` change — no new `kind`/enum values needed.

**Existing-install migration.** composer is `seedContent: "initial-only"`
(`app.json`), so the `content.json` rewrite reaches a FRESH install only —
`lib/src/composer_native_wikipick_seedmap_migration.dart`'s
`installComposerNativeWikipickSeedMapProposal` runs once from
`runLegacySeedMigrations` (`lib/src/host/legacy_migrations.dart`) to plant
the same rows into an existing store. Idempotent and existence-guarded: it
is a no-op the moment `bd_wikipick_seedmap` already exists (own board id,
brand new, so there is no prior-shape signal to check beyond existence —
"proposals plant, user edits win").
