...
## 2026-09-04.topicrome — CORE-rule chip re-verified, superseded (no schema change, no code change)

No seedVersion bump: no schema, no production code touched — test-only
(`test/wikipick_topic_capture_test.dart`).

**What this closes**: a fleet chip (`task_53a9f043`) sat open citing an
apparent contradiction between two prior live-verifies of the "rome"
search — topiccap-0902 measured Rome/Roman Empire at `linkedByCount`=3
(which the CURRENT relative CORE rule from `2026-09-03.topicthresh` below
would make CORE), while `topicthresh-0903`'s own live-verify of the same
query reported a fully degenerate pool (`topScore`=1) days later. Re-ran the
exact shipped code path (`opensearch` -> paginated `prop=links`, same
throttle/retry infra) against real `en.wikipedia.org` outside the test
harness: `topScore`=3, threshold=2, Rome and Roman Empire both CORE, all 10
Romero/Lukaku/Romeo-named isolates (Lukaku included) PERIPHERAL — exactly
the spec's own worked example. Confirmed the live "rome" opensearch result
set is stable across repeat calls today (3/3 identical), so the
`topicthresh-0903` degenerate reading is best explained by ordinary
day-to-day drift in Wikipedia's own search ranking for a short generic term,
not a pipeline defect — the derivation, threshold, pagination, and
candidate-pool selection all check out against live data. `task_53a9f043`
is closed/superseded on this evidence; no rule change proposed. Full
writeup: `_claude/projects/wikipick-v1-spec.md` W15's
"RE-VERIFIED against a stale chip" note; BOARD.md [topicrome-0904].

**Tests covering this**: new `test/wikipick_topic_capture_test.dart` case —
a REAL captured "rome" pool (live 2026-09-04) fed through
`debugTallyCentrality`/`debugIsCoreCandidate` directly, pinning the exact
CORE/PERIPHERAL split found above as a permanent regression against any
future re-litigation of this same question.
